...
## 2026-09-14 性能修复第一批（本地完成，后台事务方案待确认）
...
- `vercel.json` 准备 `regions: ["hnd1"]`，上线后函数将与东京数据库同区；
  尚未部署，当前生产仍是排查时的 iad1 部署，不声称已产生线上提速。
...
- 自动审批拒绝只读快照/移除展示排他锁/响应后恢复补丁，认为需先明确产品
  ...
  范围、边界和隔离并发验收。剩余主要风险仍是跨区串行访问（配置未上线）、
  展示读取同步恢复与锁竞争，不能把本批称为全部根因已修复。
- 待人工检查：真实登录后首次/重复跳转、Agent 历史改名删除、多标签换账号、
  Pricing 嵌套弹窗、活跃生成时切换和任务恢复；生产发布需另行授权。
