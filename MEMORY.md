...
- Video generation platform credits are calculated from the model's KIE API credits multiplied by 0.2 and rounded to the nearest integer unless a model has an explicitly documented exception. Seedance 2.0 Mini follows this shared KIE rule using the no-video-input API-credit tier for every request, including requests that contain reference videos: 480P uses 10 KIE API credits/output-second and 720P uses 22 KIE API credits/output-second before the `x 0.2` conversion and final rounding.
- Video generation offers Gemini Omni Video through KIE model `gemini-omni-video` with the unified `POST /api/v1/jobs/createTask` and `/api/v1/jobs/recordInfo` flow. The P0 surface supports text or up to seven 10 MB reference images, 16:9/9:16, 4/6/8/10 seconds, and 720P/1080P/4K. KIE flat task credits for 720P/1080P are 90/120/150/180 and for 4K are 210/240/270/300 across 4/6/8/10 seconds; Flownana applies the shared `x 0.2` conversion and rounding. Gemini source-video, uploaded-audio, and character-ID inputs are not exposed in P0 because source-video pricing uses a separate automatic-duration tier and KIE audio/character inputs require provider IDs rather than the workspace's public media URLs.
- Video generation offers Wan 3.0 Video through KIE model `wan/3-0-video` with the unified market task flow. It supports 480P/720P/1080P, Auto/16:9/9:16/1:1/4:3/3:4, 2–30 seconds, generated audio On/Off, up to ten 20 MB reference images, and a conservative P0 maximum of one 50 MB reference video plus one 15 MB reference audio. Pure one/two-image requests use first-frame/optional-last-frame fields; requests with more images or video/audio use mutually exclusive reference arrays. When a reference video is present, Flownana caps output at 15 seconds so the current 2–15 second uploader stays within KIE's 30-second input-plus-output limit. KIE API credits are 30/50/90 per output second at 480P/720P/1080P and use the shared `x 0.2` conversion and rounding.
- Video generation UI applies canonical parameter display rules across models: aspect ratio options are capped to `Auto`, `16:9`, `9:16`, `1:1`, `4:3`, `3:4`, and `21:9`; resolution options are capped to `Auto`, `480P`, `720P`, `1080P`, `2K`, and `4K`; unsupported or non-canonical provider values such as `5:4` are not shown. Fixed-duration models show fixed duration buttons, continuous whole-second ranges use the duration slider, and audio-capable models show `Auto`, `On`, and `Off` with `On` selected when switching into that model. Text-to-video `Auto` aspect ratio resolves to a provider-safe value for each provider. MiniMax, HappyHorse, and Grok image-driven requests omit aspect ratio; Seedance and Wan send `adaptive` for Auto, Seedance supports explicit `21:9`, and Gemini always sends one of its required 16:9/9:16 values.
...
- Video generation no longer offers VEO 3.1 Lite, VEO 3.1 Fast, VEO 3.1 Quality, Kling 3.0, Kling 3.0 Turbo, KIE Seedance 2/2 Fast, HappyHorse 1.0, or Bytedance Seedance 1.5 Pro. Seedance 2.0 Mini is listed through KIE only. The direct Volcengine option is hidden from the frontend, while its request/result implementation remains available for historical task polling and a future controlled relist.
- Seedance 2.0 Mini uses KIE model `bytedance/seedance-2-mini` with `POST /api/v1/jobs/createTask` and task polling at `/api/v1/jobs/recordInfo`. It offers 480P first/default and 720P, every whole second from 4–15, generated audio On by default with an Off choice, and Auto/16:9/9:16/1:1/4:3/3:4/21:9. It accepts up to 9 reference images, 3 reference videos, and 3 reference audios. Conservative upload limits are image JPEG/PNG/WebP/GIF/HEIC up to 30 MB, video MP4/MOV up to 50 MB, and audio MP3/WAV up to 15 MB. Pure one/two-image requests use KIE's first-frame/optional-last-frame mode; requests containing video, audio, or more than two images use the mutually exclusive multimodal `reference_*_urls` fields. User-selected inputs are stored as public Vercel Blob URLs so KIE can retrieve them, linked as generation inputs, and retained until the user deletes the related media. Active requests use the existing server-only `KIE_API_KEY`; the retained Volcengine code uses server-only `VOLCENGINE_ARK_API_KEY`, and provider keys must never be stored in source or client code.
- Video generation offers MiniMax H3 through KIE `minimax-h3/text-to-video` and `minimax-h3/image-to-video` at 720P and 2K for 4s through 15s. The app's 720P option maps to KIE's `768P` request value/tier. MiniMax image-to-video accepts up to two images: the first is sent as `first_frame_url` and the second as `last_frame_url`. Platform credits are calculated from KIE API credits per second (720P/768P: 18, 2K: 29) multiplied by duration and then by 0.2, rounded to the nearest integer.
- Video generation offers Grok Imagine Video 1.5 through KIE `grok-imagine-video-1-5-preview` at 480P and 720P for 1s through 15s. Grok requires an input image, so it is only exposed when the user has uploaded an image and server-side generation rejects missing image input before consuming credits. Platform credits are calculated from KIE API credits per second (480P: 14.5, 720P: 25) multiplied by duration and then by 0.2, rounded to the nearest integer.
- Video generation offers HappyHorse 1.1 through KIE `happyhorse-1-1/text-to-video` and `happyhorse-1-1/image-to-video`. HappyHorse 1.1 supports every whole second from 3s through 15s at 720P and 1080P. Platform credits are calculated from KIE API credits per second (720P: 22, 1080P: 34) multiplied by duration and then by 0.2, rounded to the nearest integer.
- Active KIE video request bodies are centralized in `lib/kie-video-request.ts` and contract-tested for Seedance 2.0 Mini, MiniMax H3, Grok Imagine Video 1.5, and HappyHorse 1.1. Seedance separates first/last-frame input from multimodal reference arrays and sends `adaptive` for Auto; MiniMax maps UI 720P to KIE `768P` and uses `first_frame_url` plus optional `last_frame_url`; HappyHorse uses `image_urls`; Grok omits the obsolete `mode` field; other image-driven KIE video requests omit `aspect_ratio`.
