...
- Video generation platform credits are calculated from the model's KIE API credits multiplied by 0.3 and rounded to the nearest integer unless a model has an explicitly documented exception. Seedance 2.0 Mini is the explicit direct-Volcengine exception: 480P costs 2 credits/second and 720P costs 3 credits/second, regardless of reference-media inputs.
...
- Video generation no longer offers VEO 3.1 Lite, VEO 3.1 Fast, VEO 3.1 Quality, Kling 3.0, Kling 3.0 Turbo, KIE Seedance 2/2 Mini/2 Fast, HappyHorse 1.0, or Bytedance Seedance 1.5 Pro. Seedance 2.0 Mini is re-listed through the direct Volcengine Ark API only.
- Seedance 2.0 Mini uses Volcengine Ark model `doubao-seedance-2-0-mini-260615` and `POST /api/v3/contents/generations/tasks`, with task detail polling at `/api/v3/contents/generations/tasks/{taskId}`. It offers 480P first/default and 720P, every whole second from 4–15, generated audio On by default with an Off choice, and Auto/16:9/9:16/1:1/4:3/3:4/21:9. It accepts up to 9 reference images, 3 reference videos, and 3 reference audios. Conservative upload limits are image JPEG/PNG/WebP/GIF/HEIC up to 30 MB, video MP4/MOV up to 50 MB, and audio MP3/WAV up to 15 MB. User-selected inputs are stored as public Vercel Blob URLs so Volcengine can retrieve them, linked as generation inputs, and retained until the user deletes the related media. The secret must be configured only as `VOLCENGINE_ARK_API_KEY`; never store provider keys in source or client code.
